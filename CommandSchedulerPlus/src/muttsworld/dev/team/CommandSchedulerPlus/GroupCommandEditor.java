		        ...
		        case 0 :
		        	...
		        	if(groupCommandEditorOption > 0 && groupCommandEditorOption <= 5) {
			        	switch(groupCommandEditorOption){ //Should move all messages here for maintainability 
			        		...
			        		case 2 : CommandCreator.showCommandGroup(sender);
		        ...
		        case 2 : //handle command replace
		        	...
		        	commandGroupInsertError(sender);
		        		...
		        		if(args[0].equals("")){
			        		...
			        		if(temp - 1 < CommandCreator.currentCommand.getCommands().size() && temp - 1 >= 0) {
			        			if((args[1].charAt(0)) == '/'){
				        		...
				        		CommandCreator.currentCommand.setCommand(Arrays.copyOfRange(args, 1, args.length), Integer.parseInt(args[0]) - 1); 
				        		groupCommandEditorOption = 0;
			        		}
			        		else {
				        		commandGroupInsertError(sender);
